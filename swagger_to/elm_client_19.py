...
def _write_query_function(fid: TextIO) -> None:
    ...
    fid.write(INDENT * 2 + 'queryParams : List Url.Builder.QueryParameter\n')
    fid.write(INDENT * 2 + 'queryParams =\n')
    fid.write(INDENT * 3 + 'List.map (\( name, value ) -> Url.Builder.string name value) params\n\n')
    fid.write(INDENT * 2 + 'filteredParams : List Url.Builder.QueryParameter\n')
    ...
    fid.write(INDENT * 4 + '|> List.map (\( name, value ) ->  Url.Builder.string name value)\n')

    fid.write(INDENT + 'in\n')
    fid.write(INDENT + 'List.concat [queryParams, filteredParams]\n')
    fid.write(INDENT * 2 + '|> Url.Builder.toQuery\n')
...
def elm_json() -> MutableMapping[str, Any]:
    """
    Generate the elm.json file.

    :return: The elm.json file as a Dictionary.
    """
    elm_json = collections.OrderedDict()  # type: Dict[str, Any]
    elm_json['type'] = 'application'
    elm_json['source-directories'] = ['src']
    elm_json['elm-version'] = '0.19.0'
    elm_json['summary'] = ''
    elm_json['repository'] = 'https://github.com/invalid/repo.git'
    elm_json['license'] = ''
    elm_json['exposed-modules'] = []

    direct_dep = collections.OrderedDict()  # type: Dict[str, str]
    direct_dep['NoRedInk/elm-json-decode-pipeline'] = '1.0.0'
    direct_dep['elm/core'] = '1.0.0'
    direct_dep['elm/http'] = '1.0.0'
    direct_dep['elm/json'] = '1.1.2'
    direct_dep['elm/url'] = '1.0.0'
    direct_dep['elm-community/json-extra'] = '4.0.0'

    elm_json['dependencies'] = collections.OrderedDict([
        ('direct', direct_dep),
        ('indirect', collections.OrderedDict([('elm/parser', '1.1.0')]))
    ])

    elm_json['test_dependencies'] = collections.OrderedDict([
        ('direct', {}), ('indirect', {})
    ])
...
def write_client_elm(typedefs: MutableMapping[str, Typedef], requests: List[Request],
                     fid: TextIO) -> None:
    """
