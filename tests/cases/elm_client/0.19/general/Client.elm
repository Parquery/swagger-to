...
encodeProductList aProductList =
    Json.Encode.object
        [ ( "products", Json.Encode.list encodeProduct <| aProductList.products )
        ]


encodeProductMap : Dict String Product -> Json.Encode.Value
encodeProductMap aProductMap =
    Json.Encode.dict identity encodeProduct <| aProductMap
...
encodePriceEstimateArray aPriceEstimateArray =
    Json.Encode.list encodeProduct <| aPriceEstimateArray
...
encodeActivities aActivities =
    Json.Encode.object
        ...
        , ( "history", Json.Encode.list encodeActivity <| aActivities.history )
        ]
...
decodeProductList =
    Json.Decode.succeed ProductList
...
decodePriceEstimate =
    Json.Decode.succeed PriceEstimate
        |> Json.Decode.Pipeline.required "product_id" Json.Decode.string
...
decodeActivities =
    Json.Decode.succeed Activities
...
-}
productsRequest : String -> Maybe Float-> Bool -> Float -> Float -> Http.Request (Dict String Product)
...
estimatesPriceRequest prefix maybeTimeout withCredentials startLatitude startLongitude endLatitude endLongitude maybeMaxLines =
    let
        baseUrl =
            prefix
                ++ "estimates/price/"
                ++ String.fromFloat startLatitude
                ++ "/"
                ++ String.fromFloat startLongitude
                ++ "/"
                ++ String.fromFloat endLatitude
                ++ "/"
                ++ String.fromFloat endLongitude

        queryString =
            paramsToQuery
                []
                [ ( "max_lines", Maybe.map String.fromInt maybeMaxLines )
                ]
...
estimatesTimeRequest prefix maybeTimeout withCredentials startLatitude startLongitude maybeCustomerUuid maybeProductID =
    let
        baseUrl =
            prefix ++ "estimates/time"

        queryString =
            paramsToQuery
                [ ( "start_latitude", String.fromFloat startLatitude )
                , ( "start_longitude", String.fromFloat startLongitude )
                ]
...
-}
updateMeRequest : String -> Maybe Float-> Bool -> Profile -> Http.Request Profile
...
-}
historyRequest : String -> Maybe Float -> Bool -> Maybe Int -> Maybe Int -> Http.Request Activities
historyRequest prefix maybeTimeout withCredentials maybeOffset maybeLimit =
    let
        ...
        queryString =
            paramsToQuery
                []
                [ ( "offset", Maybe.map String.fromInt maybeOffset )
                , ( "limit", Maybe.map String.fromInt maybeLimit )
                ]
